        ...
        <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
          <form onSubmit={submit} className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-5">
              ...
              <p className="mt-2 text-xs leading-5 text-slate-500">{mode === "standard" ? `3 个低输出请求${protocol === "anthropic" ? "，另有 2 个不生成内容的 count_tokens 请求" : ""}。` : `额外检查 ${protocol === "anthropic" ? "Tool Calling" : "Function Calling 与 Structured Output"}，会增加 ${protocol === "anthropic" ? 1 : 2} 个请求。`}</p>
            </div>
